...
def rk45_integrate(y0, t_eval, m1, m2, m3, rtol, atol):
    ...
    while next_out < n_out:
        ...
        k2 = system_odes_fast(t + h_step * 0.2, y + h_step * (_A21 * k1), m1, m2, m3)
        k3 = system_odes_fast(
...
def benchmark(t_eval):
    ...
    sol_ref, t_orig_s = _time_solve(
        t_eval=t_eval, fast=False, method="RK45", rtol=1e-9, atol=1e-9
    )
    sol_fast_s, t_fast_s = _time_solve(
        t_eval=t_eval, fast=True, method="DOP853", rtol=1e-9, atol=1e-9
    )
    print(f"  RK45  + Python RHS:  {t_orig_s * 1000:7.2f} ms")
    print(
        f"  DOP853+ fast RHS:    {t_fast_s * 1000:7.2f} ms   ({t_orig_s / t_fast_s:.2f}x)"
    )
    print()
    ...
    for substeps in (1, 4, 16):
        ...
        print(
            f"  RK4 (substeps={substeps:>2}):       {t_rk4 * 1000:7.2f} ms"
    ...
    for rtol, atol in ((1e-3, 1e-6), (1e-9, 1e-9)):
        ...
        print(
            f"  RK45 (rtol={rtol:.0e}):       {t_rk45 * 1000:7.2f} ms"
            f"   err vs gold: {err:.2e}"
        )
    ...
    print(
        f"Original main.py baseline (RK45 + Python RHS, default tol): {original_ms:.2f} ms"
    )
    return sol_fast
...
def animate_single(sol, t_eval):
    ...
    (planet1_dot,) = ax.plot(
        [p1x[-1]], [p1y[-1]], [p1z[-1]], "o", color="green", markersize=6
    )
    (planet2_dot,) = ax.plot(
        [p2x[-1]], [p2y[-1]], [p2z[-1]], "o", color="red", markersize=6
    )
    (planet3_dot,) = ax.plot(
        [p3x[-1]], [p3y[-1]], [p3z[-1]], "o", color="blue", markersize=6
    )
    ...
    _ = FuncAnimation(
        fig, update, frames=range(0, len(t_eval), 2), interval=10, blit=True
    )
    plt.show()
...
def run_parallel(n_runs=8, perturbation=1e-6):
    ...
    print(
        f"Running {n_runs} trajectories on {workers} workers (perturbation={perturbation:g})..."
    )
    t0 = time.perf_counter()
