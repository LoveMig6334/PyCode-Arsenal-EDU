...
def system_odes(t, S, m1, m2, m3):
    ...
    out = np.empty(18)
    out[0] = v1x; out[1] = v1y; out[2] = v1z
    out[3] = v2x; out[4] = v2y; out[5] = v2z
    out[6] = v3x; out[7] = v3y; out[8] = v3z
...
def rk45_integrate(y0, t_eval, m1, m2, m3, rtol, atol):
    ...
    while next_out < n_out:
        ...
        k2 = system_odes(t + h_step * 0.2,
                         y + h_step * (_A21 * k1), m1, m2, m3)
        k3 = system_odes(t + h_step * 0.3,
                         y + h_step * (_A31 * k1 + _A32 * k2), m1, m2, m3)
        k4 = system_odes(t + h_step * 0.8,
                         y + h_step * (_A41 * k1 + _A42 * k2 + _A43 * k3),
                         m1, m2, m3)
        k5 = system_odes(t + h_step * (8.0 / 9.0),
                         y + h_step * (_A51 * k1 + _A52 * k2 + _A53 * k3 + _A54 * k4),
                         m1, m2, m3)
        k6 = system_odes(t + h_step,
